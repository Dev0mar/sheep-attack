...
function Binder:Start()
	...
	self._trove:Add(CollectionService:GetInstanceAddedSignal(self._tagName):Connect(function(inst)
		self:_add(inst)
	end))
	self._trove:Add(CollectionService:GetInstanceRemovedSignal(self._tagName):Connect(function(inst)
		self:_remove(inst)
	end))
end
...
function Binder:GetClassAddedSignal()
	if not self._classAddedSignal then
		self._classAddedSignal = self._trove:Construct(Signal)
	end
...
function Binder:GetClassRemovingSignal()
	if not self._classRemovingSignal then
		self._classRemovingSignal = self._trove:Construct(Signal)
	end
...
function Binder:Destroy()
    for index, class in pairs(self._instToClass) do
        self:_remove(class)
