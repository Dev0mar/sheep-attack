...
repeat 
	task.wait()
until LocalPlayer:GetAttribute("GameStarted")

local PlayerGui = LocalPlayer:WaitForChild("PlayerGui")
local ProximityUIHolder = ReplicatedStorage.Prefabs:WaitForChild("ProximityPrompts")
...
local function createPrompt(prompt, inputType, gui)
	...
	if promptTheme then
		local themedPromptUI = ProximityUIHolder:FindFirstChild(promptTheme)
	...
	if promptUI == nil then
		promptUI = ProximityUIHolder.Default:Clone()
		print("Cloned prompt ui")
	end
	...
	local function updateUIFromPrompt()
		...
		promptUI.Size = UDim2.fromScale(1,1)
